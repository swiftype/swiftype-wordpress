...
jQuery(elem).css(type,value);},type,value,arguments.length,null);};});window.jQuery=window.$=jQuery;if(typeof define==="function"&&define.amd&&define.amd.jQuery){define("jquery",[],function(){return jQuery;});}})(window);window['$stjq']=jQuery.noConflict(true);(function(jQuery){(function($){var ident=0;window.Swiftype=window.Swiftype||{};Swiftype.root_url='http://api.swiftype.com';Swiftype.pingUrl=function(endpoint,callback){var img=new Image();img.onload=img.onerror=callback;img.src=endpoint;setTimeout(callback,350);return false;};Swiftype.pingAutoSelection=function(engineKey,entryId,value,callback){var params={t:new Date().getTime(),engine_key:engineKey,entry_id:entryId,prefix:value};var url=Swiftype.root_url+'/api/v1/public/analytics/pas?'+$.param(params);Swiftype.pingUrl(url,callback);};$.fn.swiftype=function(options){var options=$.extend({},$.fn.swiftype.defaults,options);return this.each(function(){var $this=$(this);var config=$.meta?$.extend({},options,$this.data()):options;$this.attr('autocomplete','off');$this.data('swiftype-config',config);$this.submitted=false;$this.cache=new LRUCache(10);$this.emptyQueries=[];$this.computeDropdownStyles=function(){var $attachEl=config.attachTo?$(config.attachTo):$this;var offset=$attachEl.offset();var styles={'position':'absolute','z-index':9999,'top':offset.top+$attachEl.outerHeight()+1,'left':offset.left};if(config.setWidth){styles['width']=$attachEl.outerWidth()-2;}
return styles;}
