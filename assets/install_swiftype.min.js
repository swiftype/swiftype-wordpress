...
break;case 27:$this.hideList();suppressKey=true;break;default:$this.submitted=false;break;}});$this.keypress(function(event){if((event.which==13)&&($this.activeResult().length>0)){event.preventDefault();}});$(document).bind('scroll',function(){$this.styleDropdown();});var mouseDown=false;var blurWait=false;$(document).bind('mousedown.swiftype'+ ++ident,function(){mouseDown=true;});$(document).bind('mouseup.swiftype'+ident,function(){mouseDown=false;if(blurWait){blurWait=false;$this.hideList();}});$this.blur(function(){if(mouseDown){blurWait=true;}else{$this.hideList();}});$this.focus(function(){setTimeout(function(){$this.select()},10);if($this.listResults().filter(':not(.'+config.noResultsClass+')').length>0){$list.show();}});});};var normalize=function(str){return $.trim(str).toLowerCase();};var callRemote=function($this,term){$this.abortCurrent();var params={},config=$this.data('swiftype-config');params['q']=term;params['engine_key']=config.engineKey;function handleFunctionParam(field){if(field!==undefined){var evald=field;if(typeof evald==='function'){evald=evald.call();}
return evald;}
return undefined;}
params['search_fields']=handleFunctionParam(config.searchFields);params['fetch_fields']=handleFunctionParam(config.fetchFields);params['filters']=handleFunctionParam(config.filters);params['document_types']=handleFunctionParam(config.documentTypes);params['functional_boosts']=handleFunctionParam(config.functionalBoosts);params['sort_field']=handleFunctionParam(config.sortField);params['sort_direction']=handleFunctionParam(config.sortDirection);var endpoint=Swiftype.root_url+'/api/v1/public/engines/suggest.json';$this.currentRequest=$.ajax({type:'GET',dataType:'jsonp',url:endpoint,data:params}).success(function(data){var norm=normalize(term);if(data.record_count>0){$this.cache.put(norm,data.records);}else{$this.addEmpty(norm);$this.data('swiftype-list').empty().hide();return;}
processData($this,data.records,term);});};var getResults=function($this,term){var norm=normalize(term);if($this.isEmpty(norm)){$this.data('swiftype-list').empty().hide();return;}
...
$this.lastValue=term;if($.trim(term)===''){$this.data('swiftype-list').empty().hide();return;}
if(typeof $this.data('swiftype-config').engineKey!=='undefined'){getResults($this,term);}};var processData=function($this,data,term){var $list=$this.data('swiftype-list'),config=$this.data('swiftype-config');$list.empty().hide();config.resultRenderFunction($this.getContext(),data);var totalItems=$this.listResults().length;if((totalItems>0&&$this.focused())||(config.noResultsMessage!==undefined)){if($this.submitted){$this.submitted=false;}else{$list.show();}}};var defaultResultRenderFunction=function(ctx,results){var $list=ctx.list,config=ctx.config;$.each(results,function(document_type,items){$.each(items,function(idx,item){ctx.registerResult($('<li>'+config.renderFunction(document_type,item)+'</li>').appendTo($list),item);});});};var defaultRenderFunction=function(document_type,item){return'<p class="title">'+item['title']+'</p>';};var defaultOnComplete=function(item,prefix){window.location=item['url'];};function LRUCache(limit){this.size=0;this.limit=limit;this._keymap={};}
...
$.fn.swiftype.defaults={activeItemClass:'active',attachTo:undefined,documentTypes:undefined,filters:undefined,engineKey:undefined,searchFields:undefined,functionalBoosts:undefined,sortField:undefined,sortDirection:undefined,fetchFields:undefined,noResultsClass:'noResults',noResultsMessage:undefined,onComplete:defaultOnComplete,resultRenderFunction:defaultResultRenderFunction,renderFunction:defaultRenderFunction,resultLimit:10,suggestionListType:'ul',suggestionListClass:'st-autocomplete',resultListSelector:'li',setWidth:true,typingDelay:80};$(function(){Swiftype.engineKey=swiftypeParams.engineKey;Swiftype.inputElements=$('input[name=s]');function readSwiftypeConfigFor(option){if((typeof window.swiftypeConfig==='undefined')||(typeof window.swiftypeConfig[option]==='undefined')||window.swiftypeConfig[option]===null){return undefined;}
return function(){return window.swiftypeConfig[option]};}
var SwiftypeConfigManager={getFilters:function(){return readSwiftypeConfigFor('filters');},getSearchFields:function(){return readSwiftypeConfigFor('searchFields');},getSortField:function(){return readSwiftypeConfigFor('sortField');},getSortDirection:function(){return readSwiftypeConfigFor('sortDirection');}};var swiftypeOptions={onComplete:function(dataItem,prefix){Swiftype.pingAutoSelection(Swiftype.engineKey,dataItem['id'],prefix,function(){window.location=dataItem['url'];});},documentTypes:['posts'],engineKey:Swiftype.engineKey,filters:SwiftypeConfigManager.getFilters(),searchFields:SwiftypeConfigManager.getSearchFields(),sortField:SwiftypeConfigManager.getSortField(),sortDirection:SwiftypeConfigManager.getSortDirection()};$.each(Swiftype.inputElements,function(idx,el){var $el=$(el);$el.swiftype(swiftypeOptions);});});})(jQuery);})($stjq);var script=document.createElement('script');script.type='text/javascript';script.async=true;script.src='https://swiftype.com/te.js';var entry=document.getElementsByTagName('script')[0];entry.parentNode.insertBefore(script,entry);